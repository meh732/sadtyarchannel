...
export const PromptsView: React.FC<PromptsViewProps> = ({
  ...
  const handleSendPromptDirectly = async (promptId: string, channelNum: number = 1) => {
    try {
      setSendingPromptId(promptId);
      const res = await fetch(`/api/ai-prompts/${promptId}/send-now`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok && data.success) {
        showToast(`پرامپت با موفقیت به کانال ${channelNum} ارسال شد!`, 'success');
        await onRefreshPrompts();
      } else {
        showToast(data.message || 'خطا در ارسال پرامپت به کانال', 'error');
      }
    } catch (err: any) {
                  ...
                  {token && (
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          onClick={() => handleSendPromptDirectly(p.id, 1)}
                          disabled={sendingPromptId === p.id}
                          className="flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 active:scale-95 rounded-lg transition-all cursor-pointer disabled:opacity-50"
                          title="ارسال فوری همین پرامپت به کانال تلگرام"
                        >
                          <Send className={`w-3 h-3 ${sendingPromptId === p.id ? 'animate-pulse' : ''}`} />
                          <span>{sendingPromptId === p.id ? 'در حال ارسال...' : 'ارسال به کانال'}</span>
                        </button>
                        <button
