...
export default function App() {
  ...
  // Handle Test Gemini AI Key
  const handleTestGemini = async () => {
    const keyToTest = (settings.geminiApiKey || '').trim();
    if (!keyToTest) {
      showToast('لطفاً ابتدا کلید Gemini API را وارد کنید.', 'error');
      return;
    }
    setActionLoading('test_gemini');
    try {
      ...
      if (data.success) {
        ...
        showToast(data.message || 'خطا در تست کلید هوش مصنوعی', 'error');
      }
    } catch (err: any) {
                ...
                <div key="settings" className="max-w-3xl mx-auto space-y-6">
                  ...
                  <form onSubmit={handleSaveSettings} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
                    ...
                    <div className="space-y-4">
                      {/* Google Gemini AI API Key Input */}
                      <div className="space-y-2 p-4 rounded-xl bg-gradient-to-r from-indigo-50/70 via-purple-50/50 to-slate-50 border border-indigo-100/80">
                        <div className="flex items-center justify-between">
                          <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                            <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
                            <span>کلید هوش مصنوعی گوگل (Google Gemini API Key)</span>
                          </label>
                          <a
                            ...
                            title="دریافت رایگان کلید رسمی از گوگل"
                          >
                        ...
                        <div className="relative">
                          <input
                            ...
                            className="w-full px-4 py-2.5 pl-24 rounded-xl border border-slate-200 bg-white text-sm font-mono text-left focus:border-indigo-500 focus:outline-none"
                            dir="ltr"
                          />
                          <div className="absolute left-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                            <button
                              ...
                              title={showGeminiApiKey ? 'مخفی کردن کلید' : 'نمایش کلید'}
                            >
                            ...
                            <button
                              ...
                              disabled={actionLoading === 'test_gemini' || !settings.geminiApiKey?.trim()}
                              className="px-2.5 py-1 text-[11px] font-bold text-indigo-700 bg-indigo-100 hover:bg-indigo-200 active:scale-95 rounded-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1 cursor-pointer"
                              title="تست سریع صحت کلید با فراخوانی هوش مصنوعی"
                            >
                              {actionLoading === 'test_gemini' ? (
                                <>
                                  <RefreshCw className="w-3 h-3 animate-spin" />
                                  <span>تست...</span>
                                </>
                        ...
                        <p className="text-[11px] text-slate-500 leading-relaxed">
                          این کلید برای استخراج آنلاین ترندها، سبک‌های عکس و پرامپت‌های مهندسی‌شده هوش مصنوعی در تب پرامپت‌ها و ارسال خودکار استفاده می‌شود. (در صورت خالی بودن، سیستم از بانک ترندهای پرامپت داخلی استفاده می‌کند).
                        </p>
                      </div>
